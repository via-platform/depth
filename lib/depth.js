...
module.exports = class Depth {
    ...
    constructor({omnibar}, state = {}){
        ...
        this.axis = {
            bottom: {
                basis: d3.axisBottom(this.scale.x).tickPadding(4).tickSizeOuter(0).tickSizeInner(4),
                element: this.chart.append('g').attr('class', 'x axis bottom')
            },
            left: {
                basis: d3.axisRight(this.scale.y).ticks(4).tickSizeOuter(0).tickSizeInner(4).tickPadding(4),
                element: this.chart.append('g').attr('class', 'y axis left')
            },
            right: {
                basis: d3.axisLeft(this.scale.y).ticks(4).tickSizeOuter(0).tickSizeInner(4).tickPadding(4),
    ...
    updateCrosshairValues(){
        ...
        this.flags.main.select('rect').attr('width', width).attr('transform', `translate(${-1 * (width + 2) / 2}, 0)`);
        this.flags.shadow.select('rect').attr('width', width).attr('transform', `translate(${-1 * (width + 2) / 2}, 0)`);
